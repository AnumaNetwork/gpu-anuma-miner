...
#[derive(Parser, Debug)]
#[clap(name = "anuma-miner", version, about = "A Anuma high performance CPU miner", term_width = 0)]
pub struct Opt {
    #[clap(short, long, help = "Enable debug logging level")]
    pub debug: bool,
    #[clap(short = 'a', long = "mining-address", help = "The Anuma address for the miner reward")]
    pub mining_address: String,
    #[clap(short = 's', long = "anumad-address", default_value = "127.0.0.1", help = "The IP of the anumad instance")]
    pub anumad_address: String,
    ...
    #[clap(short, long, help = "Anumad port [default: Mainnet = 12412, Testnet = 12513]")]
    port: Option<u16>,
    ...
    #[clap(
        ...
        help = "Mine even when anumad says it is not synced",
        long_help = "Mine even when anumad says it is not synced, only useful when passing `--allow-submit-block-when-not-synced` to anumad  [default: false]"
    )]
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        //self.gpus = None;
        if self.anumad_address.is_empty() {
            self.anumad_address = "127.0.0.1".to_string();
        }

        if !self.anumad_address.contains("://") {
            let port_str = self.port().to_string();
            let (anumad, port) = match self.anumad_address.contains(':') {
                true => self.anumad_address.split_once(':').expect("We checked for `:`"),
                false => (self.anumad_address.as_str(), port_str.as_str()),
            };
            self.anumad_address = format!("grpc://{}:{}", anumad, port);
        }
        log::info!("anumad address: {}", self.anumad_address);
        ...
        self.devfund_address = String::from("anuma:pzhh76qc82wzduvsrd9xh4zde9qhp0xc8rl7qu2mvl2e42uvdqt75zrcgpm00");
    ...
    fn port(&mut self) -> u16 {
        *self.port.get_or_insert(if self.testnet { 12513 } else { 12412 })
    }
