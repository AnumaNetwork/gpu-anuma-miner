...
use crate::cli::Opt;
use crate::client::grpc::AnumadHandler;
...
const WHITELIST: [&str; 4] = ["libanumacuda", "libanumaopencl", "anumacuda", "anumaopencl"];
...
async fn get_client(
    anumad_address: String,
    mining_address: String,
    mine_when_not_synced: bool,
    block_template_ctr: Arc<AtomicU16>,
) -> Result<Box<dyn Client + 'static>, Error> {
    if anumad_address.starts_with("stratum+tcp://") {
        let (_schema, address) = anumad_address.split_once("://").unwrap();
        ...
        .await?)
    } else if anumad_address.starts_with("grpc://") {
        Ok(AnumadHandler::connect(
            anumad_address.clone(),
            mining_address.clone(),
            mine_when_not_synced,
            Some(block_template_ctr.clone()),
        )
...
async fn main() -> Result<(), Error> {
    ...
    let (app, mut plugin_manager): (App, PluginManager) = anuma_miner::load_plugins(Opt::into_app(), &plugins)?;
    ...
    info!("=================================================================================");
    info!("                 Anuma-Miner GPU {}", env!("CARGO_PKG_VERSION"));
