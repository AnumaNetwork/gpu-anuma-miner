...
use crate::proto::anumad_message::Payload;
use crate::proto::rpc_client::RpcClient;
use crate::proto::{
    GetBlockTemplateRequestMessage, GetInfoRequestMessage, AnumadMessage, NotifyBlockAddedRequestMessage,
...
type BlockHandle = JoinHandle<Result<(), PollSendError<AnumadMessage>>>;
...
impl AnumadHandler {
    ...
    fn create_block_channel(send_channel: Sender<AnumadMessage>) -> (Sender<BlockSeed>, BlockHandle) {
        // AnumadMessage::submit_block(block)
            ...
            tokio::spawn(async move {
                ReceiverStream::new(recv)
                    .map(|block_seed| match block_seed {
                        FullBlock(block) => AnumadMessage::submit_block(*block),
    ...
    async fn client_send(&self, msg: impl Into<AnumadMessage>) -> Result<(), SendError<AnumadMessage>> {
        self.send_channel.send(msg.into()).await
    }

    async fn client_get_block_template(&mut self) -> Result<(), SendError<AnumadMessage>> {
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::GetInfoResponse(info) => {
                info!("Anumad version: {}", info.server_version);
                let anumad_version = Version::parse(&info.server_version)?;
                let update_version = Version::parse(VERSION_UPDATE)?;
                match anumad_version >= update_version {
            ...
            Payload::NotifyBlockAddedResponse(res) => match res.error {
                None => info!("Registered for block notifications (upgrade your Anumad for better experience)"),
